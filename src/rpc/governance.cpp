...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in digiquian.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("digiquian.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* digiquian features */
    { "digiquian",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "digiquian",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "digiquian",               "gobject",                &gobject,                true,  {} },
    { "digiquian",               "voteraw",                &voteraw,                true,  {} },
