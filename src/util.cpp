...
const char * const BITCOIN_CONF_FILENAME = "digiquian.conf";
const char * const BITCOIN_PID_FILENAME = "digiquiand.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "digiquian" is a composite category enabling all digiquian-related debug output
                if(ptrCategory->count(std::string("digiquian"))) {
...
#else
    const char* pszModule = "digiquian";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\digiquian
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\digiquian
    // Mac: ~/Library/Application Support/digiquian
    // Unix: ~/.digiquian
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "digiquian";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/digiquian";
#else
    // Unix
    return pathRet / ".digiquian";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty digiquian.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override digiquian.conf
